...
2. "unit" must be the recipe unit of that ingredient. "quantity" is the total for all {{people}} servings. Fruit and vegetables are in grams even when you think in pieces: 1 banana = 120 g, 1 apple = 150 g, 1 tomato = 120 g, 1 cucumber = 300 g, 1 pepper = 150 g, 1 potato = 150 g.
3. Plan exactly one recipe for every requested meal of every day: {{days}} days x ({{meals}}).
4. The shop sells whole packages only. 100 g of rice still costs a full 1 kg bag. So reuse the same ingredients across several recipes and prefer finishing a package over opening a new one. Target: every package you open is used in at least 2 recipes when the plan has 3 meals or more.
5. Items listed in already_at_home are free. Use them when it helps.
6. Recipes must be realistic for a student kitchen: at most 6 ingredients and 4 steps of one short sentence each, normal portion sizes. Every lunch and dinner has at least one vegetable (fresh, frozen or canned). Be brief: no introductions, no tips, no explanations outside the JSON fields.
...
10. The examples below show the format and the reasoning, not the menu. Do not copy their recipes. No recipe_name appears twice in a plan, and consecutive days do not share the same base (rice, pasta, legumes, potato, bread, eggs).
</rules>
